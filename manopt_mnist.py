...
with tf.device("/cpu:0"):
    ...
    b_hist =tf.summary.histogram("bias-hist", b)
    ...
    if not use_parameterization:
        M_hist =tf.summary.histogram("M-hist", M)
    B_hist =tf.summary.histogram("B-hist", B)
    W_hist =tf.summary.histogram("W-hist", W)
